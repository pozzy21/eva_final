...
time2 = 7200
time3 = 86400
# 'pozzy199911@gmail.com'  # 'eva-standart@yandex.ru'
mailto = 'eva-standart@yandex.ru'
...
async def start_message(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo='AgACAgIAAxkDAAMUZfa-Hqu170OXCXF4aPsYSbIxfYkAArXZMRusnrhLh7W-7dDrR-IBAAMCAAN5AAM0BA',
        ...
        caption="Только факты:\n\n\
1. У нас крупнейшее производство на Дальнем Востоке\n\n\
2. В базе лекал более 1500 моделей\n\n\
3. Мы изготавливаем более 10 000 ковриков в год \n\n\
4. Нам доверяют уже более 5 лет\n\n\
5. Подходим с душой к нашему делу❤️",
...
+7(914)5-501-502', reply_markup=wait_kb())
    ...
    if (current_state == await state.get_state()):
        ...
        msg3 = await message.answer_photo(photo='AgACAgIAAxkDAAOMZfbJXPDy9WzmS57wS059excv5TQAAh3aMRusnrhL5xH8qhiRKFkBAAMCAAN3AAM0BA', caption='Все просто: мастера заряжены, а менеджер ждет твою заявку\n\n\
Нужна помощь в подборе? Звони:\n\
+7(914)5-501-502', reply_markup=wait_kb())
...
+7(914)5-501-502', reply_markup=wait_kb())
    ...
    if (current_state == await state.get_state()):
        # await delete(msg3.chat.id, msg3.message_id)
        msg3 = await query.message.answer_photo(photo='AgACAgIAAxkDAAOMZfbJXPDy9WzmS57wS059excv5TQAAh3aMRusnrhL5xH8qhiRKFkBAAMCAAN3AAM0BA', caption='Все просто: мастера заряжены, а менеджер ждет твою заявку\n\n\
Нужна помощь в подборе? Звони:\n\
+7(914)5-501-502', reply_markup=wait_kb())
...
async def order(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer_photo(
        photo='AgACAgIAAxkDAAMVZfa-HqzWvTeoEPaG-YGuOpySA28AArbZMRusnrhLVUWs7iKi6AEBAAMCAAN3AAM0BA',
...
async def order_1(query: CallbackQuery, state: FSMContext):
    await query.message.answer_photo(
        photo='AgACAgIAAxkDAAMWZfa-H0RrrBaZZiahllZo5UJxoWAAArfZMRusnrhLzC8M9mz9TmgBAAMCAAN5AAM0BA',
...
async def order_2(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo='AgACAgIAAxkDAAMXZfa-H3m9y1OzBtS5saVyQFLjwEYAArjZMRusnrhLCoXjpG8EZK8BAAMCAAN5AAM0BA',
...
async def order_3(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo='AgACAgIAAxkDAAMZZfa-IFqeaJyhKK9AtxkJi1lBiyAAArnZMRusnrhLmpkF59TtgYsBAAMCAAN5AAM0BA',
...
async def order_5(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo='AgACAgIAAxkDAAMaZfa-IWxNtezAcb6UvGXvB0Xu_w4AArrZMRusnrhLzIBEtCItYGcBAAMCAAN3AAM0BA'
    )
    builder = InlineKeyboardBuilder()
...
async def order_6_nashi(message: Message, state: FSMContext):
    ...
    await message.answer_video(video='BAACAgIAAxkDAAMlZfa-hoHoT4TrSu65KR2rbzbyoJkAAhtEAAKsnrhLh9jU--CoQ-I0BA')
    await message.answer(
        text='Наши коврики: \n \
▫️без запаха \n \
▫️не впитывают влагу \n \
...
async def order_6_nashi(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer_video(video='BAACAgIAAxkDAAMlZfa-hoHoT4TrSu65KR2rbzbyoJkAAhtEAAKsnrhLh9jU--CoQ-I0BA')
    await query.message.answer(
        text='Наши коврики: \n \
▫️без запаха \n \
▫️не впитывают влагу \n \
...
@router.callback_query(F.data == '5_type')
async def order_6_nashi(query: CallbackQuery, state: FSMContext):
    await state.update_data(chosen_typeof='5')
    await state.update_data(chosen_typeof_2='Нет')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Выбрать тип коврика",
...
@router.callback_query(F.data == '6_type')
async def order_6_nashi(query: CallbackQuery, state: FSMContext):
    await state.update_data(chosen_typeof='6')
    await state.update_data(chosen_typeof_2='Нет')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Выбрать тип коврика",
...
@router.callback_query(F.data == '7_type')
async def order_6_nashi(query: CallbackQuery, state: FSMContext):
    await state.update_data(chosen_typeof='7')
    await state.update_data(chosen_typeof_2='Нет')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Выбрать тип коврика",
...
async def order_6(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer_photo(
        photo='AgACAgIAAxkDAAMbZfa-IlfZyF7SFha7mzEO2mrJhREAArvZMRusnrhLIIlKdCBLqc8BAAMCAAN5AAM0BA',
...
async def color_cover(query: CallbackQuery, state: FSMContext):
    ...
    if (user_data['romb_sota'] == 'Сота'):
        await query.message.answer_photo(photo='AgACAgIAAxkDAAMcZfa-Iot_EyCEyZkB4vAQoErkgWMAArzZMRusnrhLPNLo557IDf8BAAMCAAN5AAM0BA')
    else:
        await query.message.answer_photo(photo='AgACAgIAAxkDAAMcZfa-Iot_EyCEyZkB4vAQoErkgWMAArzZMRusnrhLPNLo557IDf8BAAMCAAN5AAM0BA')
    await query.message.answer(text='Выбери цвет коврика и напиши его в поле ниже', reply_markup=builder.as_markup())
...
async def cantik_color(message: Message, state: FSMContext):
    ...
    await message.answer_photo(photo='AgACAgIAAxkDAAMdZfa-I4u2g_b8C8yZ2q539y2BzW8AAr3ZMRusnrhLTLFknB1-pfwBAAMCAAN5AAM0BA')
...
async def order_9(query: CallbackQuery, state: FSMContext):
    ...
    user_data = await state.get_data()
    print(f'Клиент ввел данные:{user_data}')
    await query.message.answer_photo(photo='AgACAgIAAxkDAAMeZfa-JbszpLaOKX3JK3ayd6tzifcAAr7ZMRusnrhLGmuar_zX1VsBAAMCAAN3AAM0BA')
...
async def order_10(message: Message, state: FSMContext):
    ...
    await message.answer_photo(photo='AgACAgIAAxkDAAMgZfa-KwTXP4hBRCPNvXllCOL74wADwNkxG6yeuEt-7crGyatBzgEAAwIAA3cAAzQE')
    await message.answer(text='<b>Выбери тип подпятника</b> \n\
Зачем он вообще нужен? Надежно защищает от вмятин и трещин, увеличивает срок службы водительского коврика', reply_markup=choice_type_of_pyatka())
    user_data = await state.get_data()
    print(f'Клиент ввел данные:{user_data}')
...
async def order_10_b(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer_photo(photo='AgACAgIAAxkDAAMgZfa-KwTXP4hBRCPNvXllCOL74wADwNkxG6yeuEt-7crGyatBzgEAAwIAA3cAAzQE')
...
@router.callback_query(F.data == 'P-30')
async def order_11(query: CallbackQuery, state: FSMContext):
    await state.update_data(type_of_pyatka='П-30')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Да",
    ...
    await query.message.answer_photo(photo='AgACAgIAAxkDAAMhZfa-Lip3aTBgEzfB4FpNcZsksx0AAsLZMRusnrhLLDuOMeUhFhkBAAMCAAN3AAM0BA')
...
@router.callback_query(F.data == 'P-40')
async def order_11(query: CallbackQuery, state: FSMContext):
    await state.update_data(type_of_pyatka='П-40')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Да",
        callback_data="yes_shield")
...
@router.callback_query(F.data == 'P-20')
async def order_11(query: CallbackQuery, state: FSMContext):
    await state.update_data(type_of_pyatka='П-20')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Да",
        callback_data="yes_shield")
...
async def order_11(query: CallbackQuery, state: FSMContext):
    await state.update_data(type_of_pyatka='Не нужен')
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Да",
        callback_data="yes_shield")
...
async def order_12_summary_y(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer_photo(photo="AgACAgIAAxkDAAMiZfa-L8sFwiiG0aQjgEay81B3StUAAsPZMRusnrhLLykH8GvLfXsBAAMCAAN5AAM0BA")
...
async def order_13_final(query: CallbackQuery, state: FSMContext):
    ...
    builder.row(types.InlineKeyboardButton(
        text="Instagram", url="https://www.instagram.com/eva_standart/")
    )
...
Шильдик: {user_data['shield']}")
    await state.clear()
    await query.message.answer(text='<b>Мы свяжемся в ближайшее время для подтверждения заказа! А пока можете посмотреть другие товары в нашем Инстаграм </b>', reply_markup=builder.as_markup())
